                ...
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        ...
                        <el-button type="success" size="mini" icon="el-icon-check" circle
                            @click="handleFinishOrder(scope.row)" style="margin-left: 5px;" :disabled="isButtonDisabled">
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
        ...
        getOrderData: function () {
            //使用Ajax请求---post
            let that = this
            getOrder('未支付').then(function (res) {
                //请求成功
                if (res.data.code === 1) {
        ...
        queryOrderData() {
            ...
            let that = this
            queryOrder(that.inputstr, '未支付').then(function (res) {
                //请求成功
                if (res.data.code === 1) {
                    that.tableData = res.data.data
                    //获取当前返回记录的总行数
        ...
        handleFinishOrder(row) {
            this.$confirm('是否确认完成该订单【订单号: ' + row.O_id + '\t账户: ' + row.C_account_id + '】？', '提示', {
                ...
                //使用Ajax请求---post
                let that = this
                finishOrder(row.O_id).then(function (res) {
                    //成功
                    if (res.data.code === 1) {
                        that.tableData = res.data.data
                        //获取当前返回记录的总行数
